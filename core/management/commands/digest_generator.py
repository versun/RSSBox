...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if publish_days.lower() not in valid_days:
            self.stderr.write(
                self.style.ERROR(
                    f"Error: Invalid publish_days. Valid options: {', '.join(valid_days)}"
                )
            )
            sys.exit(1)
        ...
        if not digests:
            self.stdout.write(
                self.style.WARNING(
                    f"No active digests found for publish_days: {publish_days}"
                )
            )
            return

        self.stdout.write(
            f"Found {len(digests)} digest(s) to process for {publish_days}"
        )
