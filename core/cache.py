...
def cache_rss(feed_slug:str, feed_type="t", format="xml"):
    logging.debug(f"Start cache_rss for {feed_slug} with type {feed_type} and format {format}")
    # 生成唯一的缓存键
...
def cache_category(category:str, feed_type="t", format="xml"):
    logging.debug(f"Start cache_category for {category} with type {feed_type} and format {format}")
    # 生成唯一的缓存键
