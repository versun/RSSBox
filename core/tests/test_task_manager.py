...
class TaskManagerTestCase(TestCase):
    ...
    def test_duplicate_task_prevention(self):
        ...
        # Submit first task
        future1 = tm.submit_task("test_task", test_function, "first", 1)  # Reduce wait time
        self.assertIsNotNone(future1, "First task submission should return Future object")
        ...
        self.assertIs(future1, future2, "Same-name task should return the same Future object")
        ...
        self.assertIn(status['status'], ['running', 'completed'], "Task status should be running or completed")
        ...
        self.assertIsNotNone(future3, "Should be able to create new same-name task after completion")
        self.assertIsNot(future1, future3, "New task should be a different Future object")
        
        result3 = future3.result()
        self.assertEqual(result3, "Result from third", "New task result should be correct")
        
        # Check task list
        all_tasks = tm.list_tasks()
        self.assertIn("test_task", all_tasks, "Task list should contain the task")
        self.assertEqual(all_tasks["test_task"]['status'], 'completed', "Task status should be completed")
    ...
    def test_concurrent_different_tasks(self):
        ...
        for i in range(task_count):
            ...
            future = tm.submit_task(task_name, test_function, f"task_{i}", 1)  # Reduce wait time
            tasks.append((task_name, future))
            self.assertIsNotNone(future, f"Task {task_name} should be submitted successfully")
        ...
        self.assertEqual(len(unique_futures), task_count, "All tasks should be different Future objects")
        ...
        for i, (task_name, future) in enumerate(tasks):
            ...
            self.assertEqual(result, expected_result, f"Task {task_name} result should be correct")
            results.append(result)
        
        # Verify all results are different
        self.assertEqual(len(set(results)), task_count, "All task results should be different")
        ...
        for i in range(task_count):
            ...
            self.assertEqual(all_tasks[task_name]['status'], 'completed', f"{task_name} status should be completed")
    ...
    def test_submit_task_always_returns_future(self):
        ...
        self.assertIsNotNone(future2, "Same-name task should return existing Future object")
        self.assertIs(future1, future2, "Should return the same Future object")
        
        # Wait for task completion
        result1 = future1.result()
        self.assertIn("0.3 seconds", result1, "Task result should contain correct execution time")
        ...
        self.assertIsNotNone(future3, "Same-name task after completion should create new Future object")
        self.assertIsNot(future3, future1, "Should be a new Future object")
        
        result3 = future3.result()
        self.assertIn("0.2 seconds", result3, "New task result should contain correct execution time")
        ...
        self.assertEqual(len(futures_list), thread_count, f"Should have {thread_count} Future objects")
        self.assertTrue(all(f is not None for f in futures_list), "All Futures should not be None")
        ...
        self.assertEqual(len(unique_futures), 1, "All same-name tasks should point to the same Future object")
        
        # Wait for concurrent task completion
        if futures_list and futures_list[0]:
            result = futures_list[0].result()
            self.assertIn("0.5 seconds", result, "Concurrent task result should contain correct execution time")
