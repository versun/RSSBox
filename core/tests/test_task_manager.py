...
class TaskManagerTestCase(TestCase):
    ...
    def test_list_tasks_filtering(self):
        ...
        self.tm.submit_task("pending_task", test_function_long_running, "pending", duration=0.2)
        ...
        self.assertTrue(
            "pending_task" in pending_tasks or "pending_task" in self.tm.list_tasks("running")
        )
    ...
    def test_update_progress(self):
        ...
        # Submit a task
        future = self.tm.submit_task("progress_task", test_function_long_running, "progress")
    ...
    def test_cancel_task(self):
        ...
        future = self.tm.submit_task("cancel_task", test_function_long_running, "cancel", duration=0.2)
        ...
        completed_future = self.tm.submit_task("completed_cancel_test", test_function, "test")
    ...
    def test_get_running_and_pending_tasks(self):
        ...
        # Submit tasks
        self.tm.submit_task("running_task", test_function_long_running, "running", duration=0.2)
        self.tm.submit_task("pending_task", test_function_long_running, "pending", duration=0.2)
        ...
        task_states = [all_tasks["running_task"]["status"], all_tasks["pending_task"]["status"]]
        self.assertTrue(
            any(state in ["pending", "running"] for state in task_states)
        )
    ...
    def test_shutdown_functionality(self):
        ...
        # Submit a task
        future = self.tm.submit_task("shutdown_test", test_function_long_running, "test")
    ...
    def test_task_info_structure(self):
        ...
        # Submit a task
        future = self.tm.submit_task("info_test", test_function, "arg1", kwarg1="value1")
        
        # Get task info
        task_info = self.tm.get_task_status("info_test")
        
        # Verify all expected fields
        expected_fields = ["id", "name", "status", "result", "exception", "progress", 
                          "start_time", "finish_time", "args", "kwargs"]
    ...
    def test_concurrent_access_thread_safety(self):
        ...
        def worker(worker_id):
            try:
                # Submit task
                future = self.tm.submit_task(f"concurrent_worker_{worker_id}", test_function, f"worker_{worker_id}")
                result = future.result()
                ...
                if status["status"] != "completed":
                    errors.append(f"Worker {worker_id}: unexpected status {status['status']}")
    ...
    def test_context_manager_edge_cases(self):
        ...
        try:
            with TaskManager(max_workers=2) as tm:
                future = tm.submit_task("context_exception", test_function_with_error, "test")
                future.result()
        except ValueError:
        ...
        with TaskManager(max_workers=3) as tm:
            futures = []
            for i in range(3):
                future = tm.submit_task(f"context_multi_{i}", test_function, f"test_{i}")
    ...
    def test_successful_task_cancellation(self):
        ...
        future = self.tm.submit_task("long_cancel_test", test_function_long_running, "test", duration=1.0)
    ...
    def test_mocked_successful_cancellation(self, mock_cancel):
        ...
        # Submit a task
        future = self.tm.submit_task("mock_cancel_test", test_function_long_running, "test", duration=0.5)
