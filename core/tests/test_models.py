...
class FilterModelTest(TestCase):
    ...
    def test_apply_ai_filter_passed(self, mock_translate):
        ...
        ai_filter = Filter.objects.create(name="AI Filter", agent=agent, filter_prompt="Is this about AI?")
        mock_translate.return_value = {"text": "Passed", "tokens": 10}

        filtered_qs, _ = ai_filter.apply_ai_filter(Entry.objects.filter(id=self.entry1.id))
    ...
    def test_apply_ai_filter_blocked(self, mock_translate):
        ...
        filtered_qs, _ = ai_filter.apply_ai_filter(Entry.objects.filter(id=self.entry2.id))
    ...
    def test_apply_ai_filter_maybe(self, mock_translate):
        ...
        filtered_qs, _ = ai_filter.apply_ai_filter(Entry.objects.filter(id=self.entry3.id))
...
class OpenAIAgentModelTest(TestCase):
    ...
    def test_validate_failure(self, mock_openai_class):
        ...
        mock_client = MagicMock()
        mock_client.with_options().chat.completions.create.side_effect = Exception("API Error")
        mock_openai_class.return_value = mock_client
    ...
    def test_translate_method(self, mock_completions):
        ...
        result = self.agent.translate(text="hello", target_language="Chinese", text_type="title")
        ...
        expected_prompt = self.agent.title_translate_prompt.replace('{target_language}', "Chinese")
    ...
    def test_completions_method(self, mock_openai_class, mock_get_token_count):
        ...
        # Test failure case
        mock_client.with_options().chat.completions.create.side_effect = Exception("API Connection Error")
    ...
    def test_completions_chunking(self, mock_get_token_count, mock_adaptive_chunking, mock_init):
        ...
        mock_completion_1.choices = [MagicMock(message=MagicMock(content="Translated first."))]
        mock_completion_1.usage = MagicMock(total_tokens=20)

        mock_completion_2 = MagicMock()
        mock_completion_2.choices = [MagicMock(message=MagicMock(content="Translated second."))]
        ...
        mock_client.with_options().chat.completions.create.side_effect = lambda **kwargs: api_results.pop(0)
    ...
    def test_summarize_method(self, mock_completions):
        ...
        expected_prompt = self.agent.summary_prompt.replace('{target_language}', "English")
        mock_completions.assert_called_once_with(
    ...
    def test_digester_method(self, mock_completions):
        ...
        self.agent.digester(text="Test text", target_language="English", system_prompt=custom_prompt)
    ...
    def test_filter_method(self, mock_completions):
        ...
        result_passed = self.agent.filter(text="Test text", system_prompt="Filter this:")
        ...
        result_blocked = self.agent.filter(text="Test text", system_prompt="Filter this:")
        self.assertFalse(result_blocked['passed'])
        self.assertEqual(result_blocked['tokens'], 0) # Tokens should be 0 if not passed
...
class LibreTranslateAgentModelTest(TestCase):
    ...
    def test_translate_success(self, mock_api_translate):
        ...
        mock_api_translate.assert_called_once_with(q="Test Text", source="auto", target="zh", format="text")
...
class DeepLAgentModelTest(TestCase):
    ...
    def test_translate_failure(self, mock_translator_class):
        ...
        mock_translator_instance = MagicMock()
        mock_translator_instance.translate_text.side_effect = Exception("DeepL Translate Error")
        mock_translator_class.return_value = mock_translator_instance
...
class LibreTranslateAgentModelTest(TestCase):
    ...
    def test_translate_success(self, mock_api_translate):
        ...
        self.assertEqual(result['text'], "Translated Text")
        self.assertEqual(result['characters'], len("Test Text"))
        mock_api_translate.assert_called_once_with(q="Test Text", source="auto", target="zh", format='html')
