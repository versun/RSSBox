...
class Filter(models.Model):
    ...
    def apply_ai_filter(self, queryset):
        ...
        for entry in queryset:
            ...
            if created or self.needs_re_evaluation(result, entry):
                ...
                filter_results = self.agent.filter(text=text_str, system_prompt=self.filter_prompt)
    ...
    def save(self, *args, **kwargs):
        ...
        if not is_new and original is not None:
            ...
            need_clear_ai_filter_cache = (self.filter_method in [self.AI_ONLY, self.BOTH] and ai_fields_changed)
            # 如果有变化，清除所有相关缓存结果
...
class FilterResult(models.Model):
    ...
    passed = models.BooleanField(_("Passed Filter"), blank=True, default=None, null=True)  # 是否通过过滤
    last_updated = models.DateTimeField(auto_now=True)
    
    class Meta:
        unique_together = [('filter', 'entry')]
        indexes = [
            models.Index(fields=['filter', 'entry', 'passed'])
        ]