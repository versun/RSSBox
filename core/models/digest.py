...
class Digest(models.Model):
    ...
    tags = models.ManyToManyField(
        ...
        verbose_name=_("Tags"),
        help_text=_("Tags to include in this digest. Articles from feeds with these tags will be processed"),
    )
    ...
    summarizer = models.ForeignKey(
        ...
        help_text=_("OpenAI agent to use for generating digest content (only valid agents are shown)"),
    )
    
    days_range = models.IntegerField(
        default=1,
        verbose_name=_("Days Range"),
        help_text=_("Number of days to look back for articles (1 = today only, 7 = past week)"),
    )
    ...
    prompt = models.TextField(
        ...
        help_text=_("AI prompt for generating digest content. Use {digest_name}, {date}, {target_language} as placeholders"),
    )
    ...
    def get_articles_for_digest(self):
        ...
        entries = Entry.objects.filter(
            ...
            pubdate__gte=start_date,
            pubdate__lte=end_date
        ).distinct().order_by("-pubdate")
